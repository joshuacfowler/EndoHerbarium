...
\tom{\linelabel{R4C11a-begin}\revise{Previous work has used herbarium specimens to identify the origins and population genomics of  plant diseases such as \emph{Phytophora}, the Irish potato famine pathogen \citep{ristaino2001pcr,ristaino2002tracking,yoshida2013rise}, and have been proposed as vehicles to track other emerging plant pathogens \citep{ristaino2020importance,bradshaw2021global}.}\linelabel{R4C11a-end}}{IMO this does not fit well here, because at this point in the Intro we are zoomed in on Epichloe. I would move this sentence either to the `Natural history specimens' paragraph or the following one, where you introduce microbial symbioses.}
...
\tom{These traits contribute to highly specialized interactions between symbiont and host.}{Is this sentence necessary?}
...
We fit maximum entropy (MaxEnt) models using the maxent function in the R package dismo \citep{hijmans2017package} using the following seasonal climate predictors (1990-2020 climate normals): mean and standard deviation of \tom{spring, summer, and autumn}{Did we choose the dates by which these seasons are defined? Worth reporting?} temperature, and mean and standard deviation of spring, summer, and autumn cumulative precipitation.
...
\tom{Posterior estimates of uncertainty in spatially varying slopes indicate that these hotspots of change may have experienced increases of up to $5$\% per year while declines in prevalence may be as great as $-4$\% per year for \emph{A. hyemalis} and \emph{A. perennans}.
For \emph{E. virginicus}, uncertainty ranges between $3.5$\% increases and $2.5$\% decreases (Fig. \ref{fig:svc_time_map_CI}).}{I wonder if this information would be better placed alongside or integrated with the mean estimates described above (``In some regions...'').}
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\caption[Predicted endophyte prevalence for each host species in 1895 and 2020.]{\firstrevise{Predicted endophyte prevalence for each host species in 1895 and 2020. \firstrevise{Predictions of prevalence come from the endophyte prevalence model.} Color indicates the posterior mean endophyte prevalence for \emph{A. hyemalis} (A,D), \emph{A. perennans} (B,E), and \emph{E. virginicus} (C,F). Map lines delineate study areas and do not necessarily depict accepted national boundaries.}}
...
\tom{Spatially}{Could start new paragraph here and combine it with ``Changes in endophyte'' paragraph. I also think we might want to hedge a little more in this section to acknowledge that while there are climate signals, there are likely other drivers at play. I know this comes up in the Discussion but I think that can be briefly foreshadowed here.} variable climate trends were predictive of trends in endophyte prevalence.
...
\tom{The past indicates a resilient symbiosis, but it will be crucial to understand whether more extreme future climate conditions could tip this interaction to deteriorate.}{Interesting point but maybe redundant with what I added above about `tipping points', so maybe consolidate these ideas in one place.}}\linelabel{R2C14-end}
...
Splitting datasets can negatively impact model estimates, and the choice of how to split the data for model validation is not trivial \citep{bergmeir2012use,james2013introduction}. 
\linelabel{R4C10b-begin}\tom{From a modeler's perspective, this also urges advocacy for increased collection efforts and expansion of herbarium collections.}{I don't really understand what this is trying to say. Not much information in this sentence.}}\linelabel{R3C6-end}\linelabel{R4C10b-end}


\linelabel{R3C3-being}\revise{Another key consideration in forecasting the dynamics host-microbe symbioses is the spatial scale of both specimen georeferencing and available climate data. 
For this analysis, most specimen localities were assigned coordinates at county or city centroids, and the climate data examined was on 4 km grid cells. Georeferencing of specimens as accurately as possible is a key priority of herbarium specimen digitization efforts \citep{soltis2017digitization,davis2023herbarium}. While the INLA modeling approach that we used allows for predictions at arbitrarility small spatial scales, and would simplify connecting model predictions to the scale of a given climate driver, the course scale inherent to our analysis may obscure some local-scale ecological processes} \linelabel{R3C3-end}
Poor predictive ability at local scales in this grass-endophyte system is not surprising, as previous studies have found that local variation, even to the scale of hundreds of meters can structure endophyte-host niches \cite{kazenel2015mutualistic,gundel2024temporal}. 
...
\linelabel{R3C5-begin}\revise{\tom{Another interesting question is whether local adaption, in either host or symbiont, may influence future range shifts.}{This dangles here a little awkwardly, and I don't think it came up in the reviews...? Maybe cut?}}\linelabel{R3C5-end}
...
First and foremost, this is \firstrevise{one of a growing number of studies to examine microbial symbiosis using specimens from natural history collections, and the first, to our knowledge, to link long-term changes in the symbioses to changes in climate.}\linelabel{R1C10-end}
...
By comparing detected ecological responses with \tom{alternative mechanistic simulations of the past}{I don't really know what this means or refers to.}, we could attribute their cause, in a manner similar to methods from climate science and economics \citep{stott2010detection, carleton2016social,trenberth2015attribution}.
...
\firstrevise{\tom{Two}{Three? Four?} anonymous reviewers greatly improved an earlier version of this manuscript.}
