...
\linelabel{R2C19-begin}For example, previous researchers have \revise{examined the flowers, pollen grains, and leaves of specimens within plant collections (herbaria) collected across time} to document shifts in \revise{reproductive} phenology \citep{willis2017old, park2019herbarium,  berg2019examination}, pollination \citep{pauw2011reconstruction, duan2019century}, and herbivory \citep{meineke2019herbarium} related to anthropogenic climate change. \linelabel{R2C19-end}
...
\linelabel{R2C21-b-begin}We predicted that \revise{overall} endophyte prevalence would increase over time in tandem with climate change, and that localized hotspots of endophyte change would correspond spatially to hotspots of climate \revise{warming and drying}. \linelabel{R2C21-b-end}
...
\emph{A. hyemalis} is a small short-lived perennial species that germinates in \revise{late winter} winter and typically flowers between March and July (most common collection month: May).
...
When we identified potential endophytes with unusual morphology, low uptake of stain, or a small amount of fungal hyphae across the scored seeds, we recorded \revise{ a positive identification} for the liberal score and a negative \revise{identification for} the conservative score.
...
Collector and scorer random effects, $\chi$ and $\omega$ \revise{respectively}, \linelabel{R2C29-begin}\revise{were centered at 0 with precision parameters assigned penalized complexity (PC) priors with parameter values $U_{PC}$ = 1 and $a_{PC}$ = 0.01 \cite{simpson2017penalising}.}\linelabel{R2C29-end}
...
For this species, an increase of $100$ mm in spring precipitation \revise{was associated with } $0.6$\% per year \revise{stronger} increases in endophyte prevalence \revise{relative to regions with no change in precipitation}.
...
\linelabel{R2C38-begin}\revise{Our analysis indicated relatively high confidence that these climate drivers influence endophyte prevalence shifts in \emph{E. virginicus}}(greater than $99$\% posterior probability of either negative or positive slopes respectively), however they translate, \revise{for example}, to less than a $0.4$\% decrease in endophyte prevalence per year for \revise{each $1^oC$ of summer warming} over the century.\linelabel{R2C38-end}
...
We identified hotspots of change for \emph{A. perennans}, which was \revise{the species whose endophyte prevalence was most responsive to changes in climate drivers} (Fig. \ref{fig:climate_correlation_plot}).
...
\revise{Broad changes in the distribution and abundance of natural enemies \cite{cote2004ecological}, along with stresses from anthropogenic changes in landcover and pollution \cite{sage2020global} likely influence the benefits of symbiosis \cite{rudgers2020climate}.
...
\linelabel{R1C11-b-begin}However, symbiont-facilitated range shifts require that endophytes be present in the populations to be able to support population growth. 
...
\linelabel{R2C39-begin}\revise{We interpret this to mean that the model captures coarse-scale spatial and temporal trends reasonably well, but is not equipped to capture local-scale nuances that generate population-to-population differences.} \linelabel{R2C39-end}
...
This suggests that increasing local replication should be a factor considered in future collection efforts of natural history specimens, \revise{balancing} the required time and effort \linelabel{R1C9-begin} \revise{along with limitations on storage space within collections}. \linelabel{R1C9-end} 
