...
%\jacob{(Fig. \ref{ms-fig:svc_time_map_CI}), across the  host distributions.}{For some reason, the PDF is displaying the plot caption, which makes the sentence awkward.}
...
\response{We agree that these are interesting questions. In response to this comment we have added two new figures.
First, the new Figure 4 shows predicted endophyte prevalence across host distributions in 1895 and in 2020. % I haven't taken the time to figure out how to get ref to work correctly here. 
Interestingly, this figure shows clear expansion of the symbiosis towards the western range edge of \emph{A. hyemalis} and towards the northern range edge of \emph{A. perennans}. 
Second, as suggested by the reviewer, we also assessed the relationship between initial prevalence and predicted changes in prevalence (new Figure A11).
We found that for the two \emph{Agrostis} species, the largest increases do occur at locations with low starting prevalence. To keep the main text focused on temporal change in prevalence, we placed this figure in the supplemental material but have added text describing this to our results.}
...
\response{We have edited this sentence to include a specific prediction for how this hypothesis would relate to our study system. To summarize, theory for vertically-transmitted symbiosis suggests that the interaction should evolve to provide mutualistic benefits, which has been demonstrated for these \emph{Epichloë} symbionts under drought and herbivore stress. Under the stressors of climate change (i.e. increased drought stress), we expect that the symbionts will provide a fitness advantage and rise in prevalence within the population. We have also added specific predictions two paragraphs further down (Line \ref{ms-R2C21-b-begin}), where we present our study's central questions.}
\linesref{ms-R2C21-begin}{ms-R2C21-end}
\linesref{ms-R2C21-b-begin}{ms-R2C21-b-end}
...
\response{"Imperfect" transmission is a commonly use term within the study of symbiosis, therefore we have opted to retain its use for consistency with previous literature. We have, however, also added a definition of the term "imperfect transmission" at the point it is first used.}
...
\response{We are sorry that this terminology has caused confusion, and we appreciate the reviewer bringing this to our attention. In response to this feedback, we have made several edits to provide additional explanation. 
We record two scores for each sample to evaluate uncertainty in the process of assigning endophyte status under the microscope. In cases with unambiguous identification (presence or absence), the two scores are the same. When there is uncertainty in identification, we assign a positive ``liberal'' score and a negative ``conservative'' score for the same sample. Samples with liberal and conservative scores that differed were infrequent; only $11$\% of the samples were given differing scores. We present a comparison of results based on this scoring choice in our supplement (Fig. A8) %\ref{ms-fig:conservative_scores_plot}), 
which did not show a qualitative difference, and therefore we only present the results of the "liberal" score for the rest of the central results.}
...
\response{We have added a reference supporting the use of penalized complexity priors, and revised terminology to more closely reflect the parameter values used to implement the model in the \emph{inlabru} package.}
...
\response{We appreciate the push to improve the organization of our manuscript. In this case, we see the current placement of the section "Modeling distributions of host species" as appropriate because the preceding section describes models of endophyte prevalence that are independent from these models of host species' distributions. The predictions from the host distribution models are used principally for identifying realistic geographic ranges over which we test the relationship between climate drivers and trends in endophyte prevalence, and for visualizing relationships across the study region. The herbarium records used for endophyte surveys do not necessarily reflect the complete host species' range in the same way that GBIF records do, and so this is an important step in generating range-wide predictions of endophyte prevalence, but is not required for the mechanics of the endophyte prevalence model, which then bridges to our assessment of climate change relationships. In response to this comment, we have added additional context describing this role that the host distribution models play.}
\linesref{ms-R2C30-begin}{ms-R2C30-end}

\comment{-L281: “which” instead of “whether”?}
\response{This change would alter the meaning of the question posed in this sentence, which we feel would not accurately reflect our intention. We have opted to keep "whether".}
...
\linesref{ms-R2C39-begin}{ms-R2C39-end} Discussion
...
\response{This is an interesting question. The literature does provide some evidence of genotype-specific benefits in \emph{Epichloë} symbiosis, and it is possible that the regions that experienced strong increases in prevalence come from the rise of a particular strain of symbiont within the population. We expect this may improve host plant populations responses to global change stressors through improved fitness, but it is feasible that this would limit responses to more diverse stressors if the diversity of symbiont strains within a population is reduced. In response to this comment, we have expanded our discussion of the potential influence that host-symbiont genotype may have on our focal species' range dynamics in our re-organized discussion.}
...
\response{These figures refer to output from the endophyte prevalence model, tested against the full ``training" herbarium survey data. We have added description of the model and data for these supplemental figures (A4 and A5).}

\comment{A6; From which models this is from? How probabilities are calculated? What is A and T?}
\response{This figure refers to output from the endophyte prevalence model fit in INLA. These probability density curves are a standard output of the INLA model output to visualize parameter posterior distributions. In response to this comment, we have added description of the model that generates these parameter estimates in Fig. A6. We have also provided additional explanation of the parameters A and T, which come from Eqn. 1 describing the endophyte prevalence model.}

\comment{A7; From which models; what is the legend for the value means, what is $\tau$?}
\response{This result comes from the endophyte prevalence model. Here, the legend value shows the width of the posterior credible interval for the parameter $\tau$ which is the estimate of spatially-varying temporal trends in endophyte prevalence. In response to this comment, we have added description of the model that generates these parameter estimates in Fig. A7. We have also provided additional explanation of the parameter  $\tau$, which come from Eqn. 1 describing the endophyte prevalence model.}
...
\response{In response to this comment, we have redefined liberal vs. conservative scores in the figure caption of Fig. A8, along with adding information that describes the model from which these parameter estimates are derived. In this figure, panel A presents the posterior distributions of the global year trend for models with either liberal or conservative scores. Panel B shows the spatially-varying temporal trend for the model fit with conservative scores, comparable to Figure 3 in the main text that shows the results for models fit to the liberal score data. We have re-plotted the figure with species names in place of the four letter codes noted by Reviewer 2.}
...
\response{Thank you for pointing this out. For this figure (now Figure A12), we have re-plotted the figure with species names in place of the four letter codes noted by Reviewer 2. We have additionally edited the figure caption to more clearly describe the different sources of data (Colored points are the contemporary survey data, black crosses are herbarium survey data).}
