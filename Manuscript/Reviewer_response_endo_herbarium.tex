...
We found that for two species, the largest increases do occur at locations with low starting prevalence. To keep the main text focused on temporal change in prevalence, we placed this figure in the supplemental material but have added text describing this to our results.}
...
\response{We have edited this sentence to include a specific prediction for how this hypothesis would relate to our study system. We have also added specific predictions \tom{two paragraphs further down}{I would provide line numbers for this change.}, where we present our study's questions.}
...
\response{We appreciate the push to improve the organization of our manuscript. In this case, we see the current placement of the section "Modeling distributions of host species" as appropriate because the preceding section describes models of endophyte prevalence that are independent from these models of host species' distributions. The predictions from the host distribution models are used principally for identifying realistic geographic ranges over which we test the relationship between climate drivers and trends in endophyte prevalence, and for visualizing relationships across study region. In response to this comment, we have added additional context describing this role that the host distribution models play.\tom{}{Maybe you say this in the text but I would point out that the herbarium records do not reflect the entire species' ranges, so if we want to make range-wide predictions we need some other way to do this.}}
...
\response{This is an interesting question. \tom{}{I would add a brief response to the question here, even a quick summary of whatever speculation you added to the Discussion.}In response to this comment, we have expanded our discussion of the potential influence that host-symbiont genotype may have on our focal species' range dynamics in our re-organized discussion.}
...
\response{We have added description of the model and data for these supplemental figures (A4 and A5).\tom{}{And respond to the question here.}}
